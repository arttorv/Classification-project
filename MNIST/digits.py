...
def plot_conf_matrix(cm, error):
    ...
    plt.title(f'Confusion Matrix with error rate: {round(error*100,2)}%')
    plt.colorbar()
...
def nearest_neighbor_2(test_img, ref_images, ref_label):
...
def k_nearest_neighbor(test_img, train_images, train_label, k):
    ...
    label, ref_image, dist_vec = nearest_neighbor_2(test_img, train_images, train_label)
    # display_img(test_img)
    # print(test_img)
    for i in range(k):
        min_index = np.argmin(dist_vec)
        k_nearest.append(train_label[min_index])
        dist_vec[min_index] = 1000000
...
def RUN_classify():
    ...
    for i in range(N_test):
        k_nearest = k_nearest_neighbours(train_cluster,train_labels,test_images[i],7)
...
# PRINTING CONFUSION MATRIX OG ERROR RATE USING KNN WITH RANDOM TESTING 
def RUN_LABEL_MULTIPLE_IMAGES_KNN():
